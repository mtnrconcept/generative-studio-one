...
export interface GeneratedProject {
  ...
  files: GeneratedFile[];
  instructions: string;
  mode: ProjectGeneratorMode;
}

export type ProjectGeneratorMode = "website" | "application";
...
const applicationLayoutAddon = (projectName: string): FileMap => ({
  "/src/App.tsx": {
    code: `import React from 'react'
import './index.css'

const navigation = [
  { name: 'Tableau de bord', active: true },
  { name: 'Utilisateurs', active: false },
  { name: 'Paramètres', active: false },
];

const stats = [
  { label: 'Utilisateurs actifs', value: '1 204' },
  { label: 'Taux de conversion', value: '4,2%' },
  { label: 'Tickets ouverts', value: '18' },
];
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-gray-100">
      <div className="grid min-h-screen grid-cols-[240px_1fr]">
        <aside className="border-r border-slate-800 bg-slate-900/60 p-6">
          <h1 className="text-xl font-semibold tracking-tight">${projectName}</h1>
          <p className="mt-2 text-sm text-slate-400">Application générée automatiquement</p>
          <nav className="mt-6 space-y-1">
            {navigation.map((item) => (
              <button
                key={item.name}
                className={"w-full rounded-md px-3 py-2 text-left text-sm transition " +
                  (item.active ? "bg-indigo-600 text-white shadow-lg" : "text-slate-300 hover:bg-slate-800/80")}
              >
                {item.name}
              </button>
            ))}
        ...
        <main className="flex flex-col">
          <header className="border-b border-slate-800 bg-slate-900/40 px-8 py-5">
            <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h2 className="text-2xl font-semibold">Vue d'ensemble</h2>
                <p className="text-sm text-slate-400">Personnalise cette structure en fonction de ton application.</p>
              </div>
              <div className="flex gap-3">
                <button className="rounded-md bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-lg transition hover:bg-indigo-500">
                  Nouvelle action
                </button>
                <button className="rounded-md border border-slate-700 px-4 py-2 text-sm text-slate-300 transition hover:bg-slate-800/70">
          ...
          <section className="flex-1 space-y-8 overflow-y-auto bg-slate-950 px-8 py-8">
            <div className="grid gap-4 md:grid-cols-3">
              {stats.map((item) => (
                <div key={item.label} className="rounded-xl border border-slate-800 bg-slate-900/60 p-5">
            ...
            <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
              <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-6">
                ...
                <p className="mt-2 text-sm text-slate-400">Remplace cette section par des composants ou des graphiques adaptés.</p>
                <div className="mt-6 space-y-3 text-sm text-slate-300">
                  <div className="rounded-lg border border-slate-800 bg-slate-900/40 p-3">✔️ Mise à jour déployée</div>
                  <div className="rounded-lg border border-slate-800 bg-slate-900/40 p-3">⚙️ Maintenance planifiée</div>
                  <div className="rounded-lg border border-slate-800 bg-slate-900/40 p-3">📊 Rapport hebdomadaire généré</div>
                </div>
              </div>
              ...
              <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-6">
                ...
                <ul className="mt-4 space-y-3 text-sm text-slate-300">
                  <li className="flex items-center justify-between">
                    <span>Préparer la démo client</span>
                    <span className="rounded-full bg-indigo-600/20 px-3 py-1 text-xs text-indigo-300">Aujourd'hui</span>
                  </li>
                  <li className="flex items-center justify-between">
                    <span>Analyser les retours utilisateurs</span>
                    <span className="rounded-full bg-emerald-600/20 px-3 py-1 text-xs text-emerald-300">Demain</span>
                  </li>
                  <li className="flex items-center justify-between">
                    <span>Mettre à jour la documentation</span>
                    <span className="rounded-full bg-amber-600/20 px-3 py-1 text-xs text-amber-300">Cette semaine</span>
                  </li>
                </ul>
              </div>
            </div>
...
export const generateProjectFromPrompt = (
  prompt: string,
  mode: ProjectGeneratorMode = "website",
): GeneratedProject => {
  ...
  if (isWebsite && (lowered.includes("landing") || lowered.includes("accueil"))) {
    files = { ...files, ...landingPageAddon() };
  }

  if (isApplication) {
    files = { ...files, ...applicationLayoutAddon(projectName) };
  }
  ...
  const instructions = [
    `Projet ${isApplication ? "Application" : "Site web"} React + Vite généré pour : ${projectName}.`,
    ...
    isWebsite
      ...
      : "Structure de tableau de bord prête avec navigation latérale et cartes de statistiques.",
    lowered.includes("bouton vert")
      ? "Style du bouton principal ajusté en vert comme demandé."
      : "Couleurs par défaut conservées.",
    isApplication && (lowered.includes("tableau de bord") || lowered.includes("dashboard"))
      ? "Section chronologie et bloc À faire mis en avant pour un tableau de bord."
      : undefined,
  ]
    .filter(Boolean)
    .join("\n");
